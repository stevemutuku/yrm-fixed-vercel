...
function Label({ children }) { return <label className='text-xs font-semibold text-slate-600'>{children}</label>; }
function TextField({ label, value, onChange, required=false }) { return (<div className='flex flex-col gap-1'><Label>{label}</Label><input required={required} className='px-3 py-2 rounded-xl border focus:outline-none focus:ring w-full' value={value} onChange={e=>onChange(e.target.value)} /></div>); }
function TextArea({ label, value, onChange, rows=3 }) { return (<div className='flex flex-col gap-1 md:col-span-2'><Label>{label}</Label><textarea rows={rows} className='px-3 py-2 rounded-xl border focus:outline-none focus:ring w-full' value={value} onChange={e=>onChange(e.target.value)} /></div>); }
function SelectField({ label, value, onChange, options }) { return (<div className='flex flex-col gap-1'><Label>{label}</Label><select className='px-3 py-2 rounded-xl border focus:outline-none focus:ring w-full' value={value} onChange={e=>onChange(e.target.value)}>{options.map(opt=><option key={opt} value={opt}>{opt}</option>)}</select></div>); }
function NumberField({ label, value, onChange, min=1, max=5 }) { return (<div className='flex flex-col gap-1'><Label>{label}</Label><input type='number' min={min} max={max} className='px-3 py-2 rounded-xl border focus:outline-none focus:ring w-full' value={value} onChange={e=>onChange(Number(e.target.value))} /></div>); }
